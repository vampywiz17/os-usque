...
`usque-nativetun` 0.8.3 with both native TUN client and optional Mesh node
modes.

The current 0.8.3 source pin retains the reviewed Cloudflare Access
...
instead of reusing historical registration metadata. After a known binary-version
change, it performs one existing-key registration refresh, persists the accepted
version, and leaves tunnel startup non-blocking if the refresh must be retried.
The Cargo lockfile and crate distfiles remain checksummed. No tunnel, QUIC, MASQUE
or TUN behavior is
